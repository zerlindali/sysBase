...
public class DataRepositoryJDBCBfscrm implements DataRepository {
	...
	public DataRepositoryJDBCBfscrm(){
		
	}
	public DataRepositoryJDBCBfscrm(int clientId, String loginUserId){
		...
		try {
			...
			if (driverName != null){
				if (driverName.toLowerCase().indexOf("oracle") >=0){
					...
					databaseType ="mysql";
				}else if (driverName.toLowerCase().indexOf("sqlserver") >=0 || driverName.toLowerCase().indexOf("microsoft") >=0){
					databaseType ="mssql";
	...
	public List queryData(String filter, Map paramMap, int pageIndex, int pageSize){
		...
		List resultList = this.queryDataByPage(whereClause.toString(), aOrderBy, pageIndex, pageSize, paramObjs);
	...
	public Serializable doInsert(Map dataMap) {
		...
		SqlParameterSource sqlParameterSource = new MapSqlParameterSource(new CaseInsensitiveMap(dataMap));
		...
		try {
			...
			if ( this.isAutoKey && ExStringUtils.isNotBlank(this.masterTablePK) ){
				 result = namedJdbcTemp.update(insertSQL, sqlParameterSource, keyHolder, new String[]{this.masterTablePK});
...
/*	@Override
	public void linkColumnToValueList(){
		...
		String valuelistTable = GlobalConfig.getConfigValue("sysconfig.valuelistTable", "b_valuelist");
		String sql = "select * from b_valuelisttable where valueListName = ?";
		...
		for(Iterator<String> iter = this.columnValueListMap.keySet().iterator(); iter.hasNext();){
			...
			if (valueListMap == null || valueListMap.isEmpty()){
				List dataList = null;
				if (value != null && value.startsWith("TableValueList.")){
					Map rowMap = this.getJdbcTemplate().queryForMap(sql,  value);
					String valueSql = (String)rowMap.get("SQLTPL");
					dataList = this.getJdbcTemplate().queryForList(valueSql);
				}else if (value != null && value.startsWith("ValueList.")){
					dataList = this.getJdbcTemplate().queryForList("select keyName as optValue, valueText as optLabel from " + valuelistTable +" where typename = ?",  value.substring(10));
				}
				valueListMap = new DataObject();
				if (dataList != null){
					for(Object item : dataList){
						Map row = (Map)item;
	...
	private Object invokeCommandMethod(String methodName, Map paramMap){
		...
		try {
			Object invokeResult =  MethodUtils.invokeExactMethod(this, methodName,new Object[]{paramMap}, new Class[]{Map.class});
	...
	protected  Map getLoginUserData(){
		...
		Map userData = null;
		/*String token = (String)ExCacheUtils.getData("UserTokenCache",  "Session-" + this.loginUserId);	
